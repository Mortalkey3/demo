               ...
               <div class="row">
                   <div class="col-lg-11">
                       <h1 class="page-header">Table Retur</h1>
                       	...
                       	<a class="btn btn-sm btn-info" href="javascript:void(0)"  title="Details" onclick="refresh()">Reload</a>
										 		</div>
									 		</h1>
									 </div>
                   <!-- /.col-lg-12 -->
...
{
			$.ajax({
							url   : '<?php echo base_url('Admin/ajax_retur_detail/')?>'+po_no.replace('/','') + "/" + create_date+ "/" + cust_id,
							async : false,
							dataType : 'JSON',
...
{
			$.ajax({
							url   : '<?php echo base_url('Admin/ajax_select_retur')?>',
							async : false,
							dataType : 'JSON',
							success : function(data){
									var html = '';
