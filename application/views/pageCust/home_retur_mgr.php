...
{
			$.ajax({
							url   : '<?php echo base_url('Home/ajax_retur_detail/')?>'+po_no.split("/").join("").split(" ").join("").split("-").join("").split("&").join("") + "/"  + create_date,
							async : false,
							dataType : 'JSON',
...
{
    if(confirm('Are you sure accept this return?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('Home/ajax_accept_retur')?>/"+po_no.split("/").join("").split(" ").join("").split("-").join("").split("&").join("") + "/"  +create_date,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
{
    if(confirm('Are you sure reject this return?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('Home/ajax_cancel_retur')?>/"+po_no.split("/").join("").split(" ").join("").split("-").join("").split("&").join("") +'/' +create_date,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
{
			$.ajax({
							url   : '<?php echo base_url('Home/ajax_select_retur_mgr')?>',
							async : false,
							dataType : 'JSON',
							success : function(data){
									var html = '';
